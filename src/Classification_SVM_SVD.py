...
def main():
	...
	pca = PCA().fit_transform(density)
	
	print pca

	model = SVC().fit(pca, trainingLabel)
	print model
	
	print model.predict(pca)
	'''
	...
	grid = GridSearchCV(
	    ...
	    cv=StratifiedKFold(n_splits=3).get_n_splits(trainingSet, trainingLabel),  # what type of cross validation to use
	)
