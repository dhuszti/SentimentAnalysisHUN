...
def main():
	...
	labels = GetRatingsFromCorpus(PreprocessedCorpusPath, 0)
	...
	densifier = Densifier().fit_transform(cv)

	reddim = PCA().fit_transform(densifier)

	print reddim

	clf = LogisticRegression().fit(reddim, trainingLabel)

	print clf
	print clf.predict(reddim)
	'''
	...
	pipeline = Pipeline([
		...
		('pca', PCA()),			# this is called otherwise LSA, n_components need to have same number as input label category number
		('regression', LogisticRegression()),	# train on TF-IDF vectors w/ Naive Bayes classifier
	])
